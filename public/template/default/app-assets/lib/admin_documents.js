...
List = function List() {
    ...
    this.delete = function ($hospital, $key, $this) {
        ...
        $.ajax({
            ...
            contentType: false, // NEEDED, DON'T OMIT THIS (requires jQuery 1.6+)
            processData: false, // NEEDED, DON'T OMIT THIS
            url: $base_url + "/document/delete_document/" + $key,
            cache: false,
            success: function success(data) {
...
app.controller('CompanyController', function ($scope, $http) {
    ...
    $scope.fetch_page_content = function () {
        ...
        $http.get($base_url + '/document/fetch_documents_list').then(function (response) {
            $data = response.data;
