...
trait Wallet
{
	...
	public function scopeClearedWithin($query, array $daterange)
	{
		extract($daterange);
		$column = 'paid_at';
		return $query->whereBetween($column, ["$start 00:00:00", "$end_date 23:59:59"]);
	}
	...
	public function scopeCleared($query, $date = null, $range = null)
	{
		...
		if ($range == null) {
			...
			return $query->where('paid_at', '<=', "$today 23:59:59");
	...
	public static function availableBalanceOnUser($user_id, $category = null, $as_at = null, $daterange = null,  $balance_field = null)
	{
		...
		if ($balance_field != null) {
			//find field and return
			$balance = $balance_field['model'][$balance_field['field']] ?? null;
			if ($balance > 0 &&  @$balance_field['refresh'] != 1) {
		...
		// ->whereRaw("(expires_at is null  or expires_at > '$as_at')");
		...
		if ($balance_field != null && is_object($balance_field['model']) && method_exists($balance_field['model'], 'update')) {
			$balance_field['model']->update([$balance_field['field'] => $available]);
		}
	...
	public static function shouldCheckAvailableBalance($user_id)
	{

		$list_of_exemptions = [
			/* 1 => [
				"expires_at" => "2022-07-19"
			], */


			2381 => [ //donbay
				"expires_at" => "2022-08-01"
			]
		];


		if (!isset($list_of_exemptions[$user_id])) {
			return true;
		}


		if (time() > strtotime($list_of_exemptions[$user_id]["expires_at"])) {
			return true;
		}
	...
	public static  function createTransaction(
		...
		$check_available_balance = self::shouldCheckAvailableBalance($user_id);




		if ($type == 'debit'  &&  $check_available_balance) {
			//confirm available balance
			if ($earning_category != null) {

				$book_balance = self::availableBalanceOnUser($user_id, $earning_category, null, null, $balance_field);
		...
		try {
			...
			//decrement/increment balance field
			$balance_type = [
				"credit" => "increment",
				"debit" => "decrement",
			];
			...
			if ($balance_field != null) {
				$method = $balance_type[$type];
				$balance_field['model']->$method($balance_field['field'], $amount);
			}
