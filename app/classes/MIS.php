...
class MIS
{
	...
	public static function getDateDifference($first_date, $second_date, $type = "days")
	{
		$datetime1 = date_create($first_date);
		$datetime2 = date_create($second_date);
		$diff = date_diff($datetime1, $datetime2);
		if ($type == "hours") return $diff->format("%R%h");
		if ($type == "days") {
			return ($diff->format("%a") > 0) ? $diff->format("%R%a") . ' Days' : $diff->format("%R%h") . ' Hours';
		}
		if ($type == "months") return $diff->format("%R%m") . ' Months';
		if ($type == "years") return $diff->format("%R%y") . ' Years';
	...
	public static function compile_email($core_msg)
	{
		$controller = new \home;
		$message = $core_msg;
		$my_message_template = $controller->buildView('emails/contact-message', compact('message'), true, true);
	...
	public static function dec_enc($action, $string)
	{
		...
		$secret_key = 'Thdsesfosdvoidssdcsdsc';
	...
	public static function generate_form($data, $action, $button, $function = '', $require_confirmation = false)
	{
		...
		if ($require_confirmation == true) {
			$btn =  '<button type="button" onclick="$confirm_dialog = new DialogJS(submit_form, [this])"  class="btn btn-xs btn-primary">' . $button . '</button>';
		} else {

			$btn =  '<button type="submit"   class="btn btn-xs btn-outline-secondary">' . $button . '</button>';
		}
